 ...
 *    <div class="draggables" id="forma1" style="  
 *   position:absolute;align:center;  
 *   top:300px;right:480px;color:black;font-size:24px;"> $2ax^2+bx+c=0$</div>
...
void setup() {
  size(960,640);
  background(255);
  //println(document.getElementById("forma1").innerHTML);
  g = new game(1);
}
...
{
  int c;
  c = g.checkbuttons(mouseX, mouseY);
  if (c != 0 || g.ispaused) { g.buttonpressed(c);}
